import React from 'react';

const Experience = () => {
  return (
    <div className='py-10 bg-[#232325]' id='experience'>
      <h2 className='mb-8 text-3xl text-white text-center'>My <span>Experience</span></h2>

      <div className='mb-[20px] text-white bg-gray-700/20 p-4 rounded-3xl sm:max-w-[300] max-w-[600px] mx-auto'>
        <p>First Company</p>
        <p className='text-gray-400'>(2020 - Present)</p>
        <p className='text-gray-500'>Description of your experience with this company</p>
      </div>

      <div className='h-[50px] w-[2px] bg-slate-400 relative my-1 mx-auto'></div>

      <div className='mb-[20px] text-white bg-gray-700/20 p-4 rounded-3xl sm:max-w-[300] max-w-[600px] mx-auto'>
        <p>Second Company</p>
        <p className='text-gray-400'>(2020 - Present)</p>
        <p className='text-gray-500'>Description of your experience with this company</p>

      </div>

      <div className='h-[50px] w-[2px] bg-slate-400 relative my-1 mx-auto'></div>


      <div className='mb-[20px] text-white bg-gray-700/20 p-4 rounded-3xl sm:max-w-[300] max-w-[600px] mx-auto'>
        <p>Third Company</p>
        <p className='text-gray-400'>(2020 - Present)</p>
        <p className='text-gray-500'>Description of your experience with this company</p>

      </div>

      <div className='h-[50px] w-[2px] bg-slate-400 relative my-1 mx-auto'></div>


      <div className='mb-[20px] text-white bg-gray-700/20 p-4 rounded-3xl sm:max-w-[300] max-w-[600px] mx-auto'>
        <p>Fourth Company</p>
        <p className='text-gray-400'>(2020 - Present)</p>
        <p className='text-gray-500'>Description of your experience with this company</p>

      </div>


    </div>
  )
}

export default Experience;